...
def get_training_arguments(parse_args=True, config_path=None):
    ...
    parser.add_argument('--common.config-file', type=str,
                        default='./checkpoints/mobilevitv2-0.5.yaml')
    parser.add_argument('--dataset.category', type=str,
...
class MobileViTv2(BaseImageEncoder):
    ...
    def __init__(self, opts, *args, **kwargs) -> None:
        ...
        self.final_upsample1 = nn.Sequential(nn.ConvTranspose2d(
            out_channels, out_channels, 6, stride=4, padding=1), nn.BatchNorm2d(out_channels), nn.ReLU())
        self.final_upsample2 = nn.Sequential(nn.ConvTranspose2d(
            out_channels, out_channels, 4, stride=2, padding=1), nn.BatchNorm2d(out_channels), nn.ReLU())

        self.out_pointmaps = ConvBlock(
            out_channels, 51, 1, relu=False, bn=False)
        self.out_edgemaps = ConvBlock(out_channels, 8, 1, relu=False, bn=False)
        self.out_heatmaps = ConvBlock(
            out_channels, 51, 1, relu=False, bn=False)
...
def mobile_vit_v2():
    opts = get_training_arguments(
        config_path='./checkpoints/mobilevitv2-0.5.yaml')
